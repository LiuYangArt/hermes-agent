...
团队模式的入站消息统一使用当前应用的 `open_id`。管理员名单只决定共享资产操作权限，不能替代聊天入口的配对授权；已有以另一种用户 ID 保存的配对记录也不会授权该 `open_id`。部署校验会以正式容器的 `hermes` 用户只读核对管理员能否通过聊天入口，缺少授权时明确失败，不会自动添加配对或扩大群访问范围。
...
每轮请求使用同一条消息显示工具进度和最终文字结果，完成后不保留独立的技能调用历史。没有进度时正常发送；超长答案第一段复用原消息，其余段落与附件仍按原流程发送。进度溢出只更新最近部分，不额外留下历史消息。取消、失败和等待本人确认时更新为对应状态；审批卡片保留按钮与身份校验。
